...
function M.invert(color)
  ...
  elseif type(color) == "string" then
    local hsluv = require("teide.hsluv")
    if color ~= "NONE" then
      local hsl = hsluv.hex_to_hsluv(color)
      hsl[3] = 100 - hsl[3]
      if hsl[3] < 40 then
        hsl[3] = hsl[3] + (100 - hsl[3]) * M.light_brightness
      end
...
function M.cache.file(key)
  return vim.fn.stdpath("cache") .. "/teide-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
  ---@type boolean, teide.Cache
